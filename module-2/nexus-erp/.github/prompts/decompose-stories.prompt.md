...
You are decomposing a single Epic into implementation-ready User Stories for NexusERP.
...
4. Each story must:
   ...
   - Include **3–5 specific, testable acceptance criteria** (GIVEN/WHEN/THEN preferred).
...
6. If the story touches the AI Memory Bank, include a "Memory Bank Notes" sub-section
   specifying the memory type written/read (`decision | pattern | preference | fact | alert`).
7. Name output files using this convention:
   `specs/stories/STORY-{epic-slug}.{number}-{name}.md`
